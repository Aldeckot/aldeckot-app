...
module.exports = async (request, response) => {
  ...
  try {
    ...
    const userCode = normalizeUserCode(process.env.ALDECKOT_BOOTSTRAP_ADMIN_CODE || email.split('@')[0]);
    const password = process.env.ALDECKOT_BOOTSTRAP_ADMIN_PASSWORD;
    if (name.length < 3 || !/^\S+@\S+\.\S+$/.test(email) || !userCodeValid(userCode) || !passwordValid(password)) {
    ...
    const currentProfile = user?.id ? await requestSupabase(config, `/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&select=id,user_code,role,status&limit=1`) : [];
    if (currentProfile?.[0]?.role === 'admin' && currentProfile[0].status === 'active' && currentProfile[0].user_code === userCode) {
    ...
    if (!user) {
      user = await requestSupabase(config, '/auth/v1/admin/users', {
        method: 'POST',
        body: JSON.stringify({ email, password, email_confirm: true, user_metadata: { full_name: name, user_code: userCode } })
      });
    } else {
    ...
    await requestSupabase(config, '/rest/v1/profiles?on_conflict=id', {
      ...
      body: JSON.stringify({ id: user.id, display_name: name, full_name: name, user_code: userCode, email, role: 'admin', status: 'active' })
    });
