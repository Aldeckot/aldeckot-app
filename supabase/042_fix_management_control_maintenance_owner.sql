-- ALDECKOT | Correção da criação automática Gestão TI -> Controle TI
-- Execute após 041_management_control_maintenance_sync.sql.
--
-- A primeira versão da automação dependia de auth.uid() ao ser executada pelo
-- banco. Em uma reconciliação no SQL Editor não há uma sessão de usuário e,
-- portanto, o item automático não recebia owner_id. Esta correção usa o
-- proprietário do próprio PC/tabela, permitindo que terminais como ESTOQUE 03
-- sejam incluídos com segurança e que os registros já em manutenção sejam
-- reconciliados ao final.
...
begin
  -- A base corporativa centralizada não usa owner_id. Nela, a 043 assume a
  -- sincronização; esta etapa deve terminar sem erro para que a sequência de
  -- migrações continue válida.
  if not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'module_records' and column_name = 'owner_id'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'module_tables' and column_name = 'owner_id'
  ) or not exists (
    select 1 from information_schema.columns
    where table_schema = 'public' and table_name = 'control_items' and column_name = 'owner_id'
  ) then
    return false;
  end if;
  ...
  select record_row.owner_id
  into management_owner_id
  ...
  if management_owner_id is null then
    return false;
  end if;

  -- A última tabela criada do Controle TI pertence ao mesmo usuário do PC.
  select table_row.id, table_row.owner_id
  into latest_control_table_id, latest_control_owner_id
  from public.module_tables table_row
  where table_row.module = 'control'
    and table_row.owner_id = management_owner_id
  order by table_row.created_at desc, table_row.id desc
  limit 1;

  if latest_control_table_id is null or latest_control_owner_id is null then
    return false;
  end if;
  ...
  select item.id, item.status, item.management_record_id, item.owner_id
  into existing_control_id, existing_control_status, existing_control_management_id, existing_control_owner_id
  from public.control_items item
  where item.owner_id = management_owner_id
    and (
      item.management_record_id = p_management_record_id
  ...
  if existing_control_id is not null then
    ...
    insert into public.control_item_logs (control_item_id, owner_id, action, message)
    values (existing_control_id, existing_control_owner_id, 'update', sync_message);
    return true;
  end if;
  ...
  insert into public.control_item_logs (control_item_id, owner_id, action, message)
  values (control_item_id, latest_control_owner_id, 'create', sync_message);
  return true;
end;
$$;
...
-- Para a volta Controle TI -> Gestão TI, o vínculo automático é a chave
-- preferencial; TAG e série continuam como compatibilidade com itens antigos.
...
begin
  ...
  for matched_record in
    ...
    where table_row.module = 'management'
      and record_row.owner_id = new.owner_id
      and (
...
declare
  ...
  next_payload jsonb;
begin
  select id, owner_id, tag, serial, management_record_id
  into control_item
  ...
  for matched_record in
    ...
    join public.module_tables table_row on table_row.id = record_row.table_id
    where table_row.module = 'management'
      and record_row.owner_id = control_item.owner_id
      and (
