(() => {
  ...
  function showSecurity(message = '', kind = '') {
    selectTab('security');
    const profile = state.profile;
    content.innerHTML = `<div class="settings-layout"><section class="settings-panel-card wide"><header class="panel-heading"><div><h2>Segurança e acesso</h2><p>Atualize sua identificação, e-mail ou senha. A senha nunca é exibida nem registrada no histórico.</p></div><span class="option-status">Conta protegida</span></header><div class="panel-body">${message ? notice(message, kind) : ''}<form id="accountForm" class="settings-form"><label><span>Nome de usuário</span><input name="fullName" required minlength="3" value="${escape(profile.full_name)}"></label><label><span>E-mail corporativo</span><input name="email" type="email" required value="${escape(profile.email)}"></label><label><span>Nova senha</span><input name="password" type="password" minlength="8" autocomplete="new-password" placeholder="Deixe em branco para manter a atual"></label><button class="settings-save" type="submit">Salvar alterações de segurança</button></form></div></section>${panel('Políticas da conta', 'Proteções aplicadas ao acesso atual.', `<div class="settings-option-list"><div class="settings-option"><span class="option-icon">♢</span><span class="option-copy"><b>Sessão autenticada</b><span>O acesso é validado antes de carregar os dados do sistema.</span></span><span class="option-status">Ativa</span></div><div class="settings-option"><span class="option-icon">◌</span><span class="option-copy"><b>Permissões por perfil</b><span>${state.isAdmin ? 'Sua conta possui administração integral.' : 'Sua conta possui as permissões atribuídas pelo administrador.'}</span></span></div></div>`)}</div>`;
